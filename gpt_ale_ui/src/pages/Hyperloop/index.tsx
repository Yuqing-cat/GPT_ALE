...
import API from '@/api'
import { Category, ChoicePoint, Point } from '@/api/gpt_ale/interface'
...
const Hyperloop = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const detail = await API.gpt_ale.runStatus(id!)
      ...
      if (detail.can_annotate) {
        const choicePoints = await API.gpt_ale.annotationTarget()
        ...
        const allPoint = await API.gpt_ale.jobPointCloud(id!)
  ...
  const onStartHyperloop = () => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await API.gpt_ale.updateAnnotation(
            id!,
